...
namespace ShipMvp.Integration.SemanticKernel.Infrastructure
{
    public class SemanticKernelService : ISemanticKernelService, IDisposable
    {
            ...
            ILogger<SemanticKernelService> logger)
        {
            ...
            logger.LogInformation("Loaded OpenAI configuration: Key={KeyLength}, Deployment={Deployment}", openAIKey.Length, openAIDeployment);
            ...
            logger.LogInformation("Loaded Gemini configuration: Key={KeyLength}, Model={Model}", geminiKey?.Length ?? 0, geminiModel);
            ...
            if (!string.IsNullOrWhiteSpace(geminiKey))
            {
                kernelBuilder.AddGoogleAIGeminiChatCompletion(
                    ...
                    apiKey: geminiKey,
                    serviceId: "gemini");
                logger.LogInformation("Added Google Gemini Chat Completion to Kernel");
            }
            ...
            logger.LogInformation("SemanticKernelService constructor completed");
        }
        ...
        public async Task<FunctionResult> InvokeGenerateDraftAsync(string websiteContext, string customPrompt,
         byte[] fileBytes, string mimeType)
        {
            var items = BuildContentItems(fileBytes, mimeType);

            var arguments = new KernelArguments
            {
                ["customPrompt"] = customPrompt,
                ["websiteContext"] = websiteContext,
                ["attachmentPreview"] = items.preview
            };

            var result = await InvokeAsync("EmailGenerator", "GenerateEmailDraft", arguments);
...
#pragma warning restore SKEXP0050
        ...
        private static (ChatMessageContentItemCollection items, string preview) BuildContentItems(
            byte[] data, string mime)
        {
            ...
            {
                items.Add(new ImageContent(data, mime));
                preview = "[inline image]";
            }
            else if (mime == "text/plain")
            {
                preview = Encoding.UTF8.GetString(data);
                preview = preview.Length > 1_000 ? preview[..1_000] + " …" : preview;
            }
            else if (mime == "application/pdf")
            {
                // Extract readable text instead of base64
                string text;
                ...
                using (var doc = PdfDocument.Open(ms))
                {
                    ...
                    foreach (var page in doc.GetPages())
                    {
                        ...
                        if (sb.Length > 8000) break; // cap for safety
                    }
                    text = sb.ToString();
                }

                // If OCR is needed (scanned PDFs), run OCR before this step or fall back gracefully
            ...
            else if (mime == "application/json")
            {
                try
                {
                    ...
                    preview = jsonString.Length > 2_000 ? jsonString[..2_000] + " …" : jsonString;
                }
                catch
                {
                    preview = "[JSON file - unable to parse as text]";
                }
            }
            else if (mime == "text/csv" || mime == "application/csv")
            {
                try
                {
                    var csvString = Encoding.UTF8.GetString(data);
                    preview = csvString.Length > 2_000 ? csvString[..2_000] + " …" : csvString;
                }
                catch
                {
                    preview = "[CSV file - unable to parse as text]";
                }
            }
            else
            {
                throw new NotSupportedException($"Unsupported MIME {mime}");
            }
